package com.scofieldchang.fpvhud.fpv;
...
public class FPVDemoApplication extends Application{
    ...
    private static String file_name = "FPVDemoApplication.txt";

    private static FileService file;

//    private static GetFlightInfo getFlightInfoThread;
    ...
    public static synchronized void setFlightInfoThread(GetFlightInfo getFlightInfo){
//        getFlightInfoThread = getFlightInfo;
    }
    public static void setFile_service(FileService file_record){
        file = file_record;
    }
    ...
    private DJISDKManager.DJISDKManagerCallback mDJISDKManagerCallback = new DJISDKManager.DJISDKManagerCallback() {
        ...
        public void onGetRegisteredResult(DJIError error) {
            if(error == DJISDKError.REGISTRATION_SUCCESS) {
                try {
                    saveLog(new Date() + "onGetRegisteredResult success");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                try {
                    saveLog("onGetRegisteredResult failed\n");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                Handler handler = new Handler(Looper.getMainLooper());
        ...
        public void onProductChanged(DJIBaseProduct oldProduct, DJIBaseProduct newProduct) {
            try {
                saveLog(new Date() + "product change\n");
            } catch (Exception e) {
                e.printStackTrace();
            }
            mProduct = newProduct;
    ...
    private DJIBaseProductListener mDJIBaseProductListener = new DJIBaseProductListener() {
        ...
        public void onComponentChange(DJIComponentKey key, DJIBaseComponent oldComponent, DJIBaseComponent newComponent) {
            try {
                saveLog(new Date() + "component change\n");
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        public void onProductConnectivityChanged(boolean isConnected) {
            try {
                saveLog(new Date() + "product connectivity changed\n");
            } catch (Exception e) {
                e.printStackTrace();
            }
            if(isConnected){
//                getFlightInfoThread.connectionEstablish(isConnected, (DJIAircraft) getProductInstance());
            }else{
//                getFlightInfoThread.connectionEstablish(isConnected, null);
            }
    ...
    private DJIComponentListener mDJIComponentListener = new DJIComponentListener() {
        ...
        public void onComponentConnectivityChanged(boolean isConnected) {
            notifyStatusChange();
            try {
                saveLog(new Date() + "onComponentConnectivityChanged: " + isConnected + "\n");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
